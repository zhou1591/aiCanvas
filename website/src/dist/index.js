(function (global, factory) {
  ...
  var EventLayer = /*#__PURE__*/function (_Layer) {
    ...
    _createClass(EventLayer, [{
      ...
      key: "spaceClosePoly",
      value: function spaceClosePoly(e) {
        var _config;

        var mapMode = this.map.mode;
        var config = (_config = {}, _defineProperty$1(_config, EMapMode.Polyline, this.handlePolylineEnd), _defineProperty$1(_config, EMapMode.Polygon, this.handlePolygonEnd), _config);
        var apiName = config[mapMode];
        if (!apiName) return;
        this.tmpPointsStore.push(this.startPoint || {});
        apiName.call(this, e);
      ...
      value: function handlePanWhenDrawing(e) {
        ...
        var panScreenDistance = 20; // 每次平移20个像素
    ...
    */
    ...
    _createClass(Text, [{
      ...
      value: function refresh() {
        ...
        if (!((_this$layer4 = this.layer) !== null && _this$layer4 !== void 0 && _this$layer4.map)) {
          return;
        }
  ...
  var Map = /*#__PURE__*/function () {
    ...
    */
    ...
    _createClass(Map, [{
      ...
      key: "getZoom",
      value: function getZoom(zoom) {
        return isNumber_1(zoom) ? zoom : this.zoom;
      } // 设置实际坐标系center
    ...
    */
    ...
    _createClass(ImageLayer, [{
      ...
      value: function drawGrid() {
        ...
        var rows = get_1(this.grid, 'rows', []); // 取列和宽


        var columnsNum = get_1(this.grid, 'columnsNum', 0);

        var rowsNum = get_1(this.grid, 'rowsNum', 0); // 绘制列
        // const columnsCount = columns.length;


        var columnItemWidth = width / (columnsNum + 1); // _forEach(columns, (column: IGridItemInfo, index: number) => {
        // 这里改成了自定义列 !!!

        for (var index = 1; index < columnsNum; index++) {
          var _ref = columns[index] || {},
              _ref$color = _ref.color,
              lineColor = _ref$color === void 0 ? '#F56C6C' : _ref$color,
          ...
          var startPoint = this.map.transformGlobalToScreen({
            x: itemX,
            y: itemTopY
          });
          var endPoint = this.map.transformGlobalToScreen({
            x: itemX,
            y: itemBottomY
          });
          Graphic.drawLine(this.canvasContext, {
            start: {
              x: Math.round(startPoint.x * dpr),
              y: Math.round(startPoint.y * dpr)
            },
            end: {
              x: Math.round(endPoint.x * dpr),
              y: Math.round(endPoint.y * dpr)
            }
          }, {
        ...
        var rowItemHeight = height / (rowsNum + 1); // 这里改成了自定义列 !!!

        for (var _index = 1; _index < rowsNum; _index++) {
          // _forEach(rows, (row: IGridItemInfo, index: number) => {
          var _ref2 = rows[_index] || {},
              _ref2$color = _ref2.color,
              _lineColor = _ref2$color === void 0 ? '#F56C6C' : _ref2$color,
              _ref2$width = _ref2.width,
              _lineWidth = _ref2$width === void 0 ? 1 : _ref2$width;

          var totalItemHeight = (_index + 1) * rowItemHeight;
          ...
          var _startPoint = this.map.transformGlobalToScreen({
            x: itemLeftX,
            y: itemY
          });

          var _endPoint = this.map.transformGlobalToScreen({
            x: itemRightX,
            y: itemY
          });
          ...
          Graphic.drawLine(this.canvasContext, {
            start: {
              x: Math.round(_startPoint.x * dpr),
              y: Math.round(_startPoint.y * dpr)
            },
            end: {
              x: Math.round(_endPoint.x * dpr),
              y: Math.round(_endPoint.y * dpr)
            }
          }, {
